...
public class Base {
    ...
    public void setup() throws AWTException {
        ...
        ScreenshotListener.setDriver(driver); // Optional: if you're using ScreenshotListener
    ...
    // ✅ Login method that can be reused via dependency or directly
    @Test(priority = 0)
    public void Test_Sucessfull_Login() {
        driver.get("https://test1dns.wincloudpms.net/WinLogin/Login/");
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("ProptyText"))).sendKeys("dubaidemo" + Keys.ENTER);
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("UserName"))).sendKeys("wincloud" + Keys.ENTER);
        ...
        Assert.assertTrue(logo.isDisplayed(), "❌ Login failed: Wincloud logo not displayed.");
    }

    // ✅ Type character using Robot
    public void typeCharWithRobot(char ch) {
        int keyCode = getKeyCode(ch);

        if (keyCode == -1) {
            throw new IllegalArgumentException("❌ Cannot type unsupported character: " + ch);
        }
    ...
    // ✅ Utility getters for other classes if needed
